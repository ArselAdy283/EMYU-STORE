...
// Ambil id item dari POST/GET
$id_item = $_POST['id_item'] ?? $_GET['id_item'] ?? 0;
...
<div class="text-center mb-6">
    <h1 class="text-3xl font-bold text-[color:#db2525]">Edit Item</h1>
    <p class="text-lg text-white"> <?= htmlspecialchars($item['nama_game']); ?></p>
</div>

<div class="w-full max-w-2xl mx-auto flex flex-col items-center p-6 md:flex-row gap-6 bg-[color:#1f1f1f] rounded-lg">
    <!-- Gambar Item -->
    ...
    <div class="flex flex-col justify-center w-full md:w-2/3">
        <form method="POST" action="proses_edit_item.php" class="flex flex-col gap-4">
            ...
            <button type="submit"
                class="mt-4 bg-[color:#db2525] hover:bg-red-700 px-6 py-2 rounded-full text-black font-semibold transition">
                Simpan Perubahan
            </button>
        </form>
    </div>
